...
function showNextQuestion(event) {
    ...
    currentQDiv.addEventListener("animationend", function () {
        currentQDiv.classList.remove("fade-in", "fade-out", "hidden", "no-interaction");
    ...
    if (currentQuestion <= 28) {
        ...
        document.getElementById("progress-bar").style.width = (currentQuestion / 28 * 100) + "%";
...
function showPreviousQuestion(event) {
    ...
    if (currentQuestion <= 1) {
        ...
        document.getElementById(allSections["diagnosis_head"]).style.display = "block";
        var section = document.querySelector('section[data-section-id="topbar1_mobile"]');
    ...
    if (currentQuestion <= totalQuestions) {
        const nextQDiv = document.getElementById(`question-${currentQuestion}`);
        document.getElementById("progress-bar").style.width = (currentQuestion / 28 * 100).toString() + "%"
        setTimeout(() => {
            nextQDiv.classList.remove("hidden");
            nextQDiv.classList.add("fade-in");
            nextQDiv.style.display = "block"; // Ensure the element is displayed
...
async function ShowTime() {
    ...
    tickLogo.addEventListener("animationend", function () {
...
async function tellResult(language, rawResult) {
    ...
    var goodbrief = "We are happy your result are very good. Continue your wellbeing and keep a good shape!";
    ...
    switch (true) {
        case ((depression == "Very Severe" || anxiety == "Very Severe" || stress == "Very Severe")):
            dass = "Very Severe";
            break;
        case ((depression == "Severe" || anxiety == "Severe" || stress == "Severe")):
            dass = "Severe";
            break;
        case ((depression == "Moderate" || anxiety == "Moderate" || stress == "Moderate")):
            dass = "Moderate";
            break;
    ...
    switch (true) {
        case ((dass == "Very Severe" || dass == "Severe") || (dass == "Moderate" && (lifesatisfaction == "Very Dissatisfied" || lifesatisfaction == "Dissatisfied"))):
            result_msg = "You are a very unhappy person. Your mental health falls under the category of “psychological distress”.";
            bar_value = "Extremely Unhappy"
            break;
        case (dass == "Moderate" || (dass == "Mild" && (lifesatisfaction == "Very Dissatisfied" || lifesatisfaction == "Dissatisfied"))):
            result_msg = "You are a very unhappy person. Your mental health is in a “low-to-medium subhealth state”.";
            bar_value = "Very Unhappy"
            break;
        case (dass == "Mild" || (dass == "Normal" && (lifesatisfaction == "Very Dissatisfied" || lifesatisfaction == "Dissatisfied"))):
            result_msg = "You are a moderately unhappy person. Your mental health is in a “moderate subhealth state”.";
            bar_value = "Moderately Unhappy"
            break;
        case (dass == "Normal" && lifesatisfaction == "Neutral"):
            result_msg = "You are a slightly unhappy person. Your mental health is in a “middle to upper subhealth state”.";
            bar_value = "Slightly Unhappy"
            break;
        case (dass == "Normal" && (lifesatisfaction == "Satisfied" || lifesatisfaction == "Very Satisfied")):
            result_msg = "You are a happy person and you are satisfied with your current life. Your mental health is a “healthy state”.";
            bar_value = "Happy"
            break;
    }
    ...
    switch (true) {
        case (lifesatisfaction == "Very Dissatisfied"):
            result_msg_lifesatisfaction = "You feel very dissatisfied with life in general. ";
            break;
        case (lifesatisfaction == "Dissatisfied"):
            result_msg_lifesatisfaction = "You feel dissatisfied with life in general. ";
            break;
        case (lifesatisfaction == "Neutral"):
            result_msg_lifesatisfaction = "Your overall life satisfaction is neutral. ";
            break;
    ...
    addPara(elObj, "", false, true, create_score_bar(['Happy', 'Slightly Unhappy', 'Moderately Unhappy', 'Very Unhappy', 'Extremely Unhappy'], bar_value, "happy", "sad"));

    result_msg_2 = result_msg_lifesatisfaction + result_msg_depression + result_msg_anxiety + result_msg_stress
    ...
    table_cell.appendChild(create_score_bar(['Very Satisfied', 'Satisfied', 'Neutral', 'Dissatisfied', 'Very Dissatisfied'], lifesatisfaction, "happy", "lifesatisfaction"));
    ...
    table_cell.appendChild(label);
    table_cell.appendChild(create_score_bar(['Normal', 'Mild', 'Moderate', 'Severe', 'Very Severe'], anxiety, "happy", "anxiety"));
    table_row.appendChild(table_cell);
    ...
    table_cell.appendChild(label);
    table_cell.appendChild(create_score_bar(['Normal', 'Mild', 'Moderate', 'Severe', 'Very Severe'], depression, "happy", "depression"));
    table_row.appendChild(table_cell);
    ...
    table_cell.appendChild(label);
    table_cell.appendChild(create_score_bar(['Normal', 'Mild', 'Moderate', 'Severe', 'Very Severe'], stress, "happy", "stress"));
    table_row.appendChild(table_cell);
...
function diagnosis(lookType, lookScore) {
    ...
    const diagLevel2 = ['Very Dissatisfied', 'Dissatisfied', 'Neutral', 'Satisfied', 'Very Satisfied'];
...
function doSomething() {
    var depression = 2 * (parseInt(q8.value) + parseInt(q10.value) + parseInt(q15.value) + parseInt(q18.value) + parseInt(q21.value) + parseInt(q22.value) + parseInt(q26.value));
    var anxiety = 2 * (parseInt(q7.value) + parseInt(q9.value) + parseInt(q12.value) + parseInt(q14.value) + parseInt(q20.value) + parseInt(q24.value) + parseInt(q25.value));
    var stress = 2 * (parseInt(q6.value) + parseInt(q11.value) + parseInt(q13.value) + parseInt(q16.value) + parseInt(q17.value) + parseInt(q19.value) + parseInt(q23.value));
    var lifesatisfaction = parseInt(q1.value) + parseInt(q2.value) + parseInt(q3.value) + parseInt(q4.value) + parseInt(q5.value);
    ...
    var allResult = [depressionResult, anxietyResult, stressResult, lifeSatisfactionResult, isParent, hasFriendsWithMentalConncern];
...
async function ShowIt() {
    ...
    var resultStatementObj = document.getElementById(allSections["diagnosis_result"]);
    ...
    if (resultStatementObj.style.display == "none") resultStatementObj.style.display = "block";
    //
    //
...
function showSuggestCourseSection(suggestedCourseKey) {
    suggestedCourseKey.forEach(key => document.getElementById(suggestedCourseSection[key]).style.display = "block");
}
