...
class QuestionNavigator {
  constructor () {
    // Initialize current question and total questions
    this.currentQuestion = 1
    this.totalQuestions = 28

    // Cache main elements
    this.progressBar = document.getElementById('progress-bar')
    this.nextButton = document.getElementById('next_step_button')
    this.prevButton = document.getElementById('pre_qusestion_button')

    // Bind the showQuestions method to the start button
    const startButton = document.querySelector(
      'input[value="Start assessment"]'
    )
    if (startButton) {
      startButton.addEventListener('click', () => this.showQuestions())
    }

    // Bind choice buttons
    const buttons = document.querySelectorAll('.choice-btn')
    buttons.forEach(button => {
      button.addEventListener('click', event => this.handleChoiceClick(event))
    })
  }
  ...
  showQuestions () {
    ...
    document.getElementById(allSections.diagnosis_questions).style.display =
      'block'
    ...
    // Hide mobile topbar if it exists
  ...
  handleChoiceClick (event) {
    ...
    setTimeout(() => {
      ...
      const questionButtons =
        button.parentElement.querySelectorAll('.choice-btn')
  ...
  async showNextQuestion (event) {
    ...
    await this.fadeOutElement(currentQDiv)
    ...
    if (this.currentQuestion <= this.totalQuestions) {
      const nextQDiv = document.getElementById(
  ...
  async showPreviousQuestion (event) {
    if (event) event.preventDefault()
    ...
    currentQuestion--
    const prevQDiv = document.getElementById(`question-${this.currentQuestion}`)
    this.progressBar.style.width =
      (this.currentQuestion / this.totalQuestions) * 100 + '%'

    setTimeout(() => {
      prevQDiv.classList.remove('hidden')
      prevQDiv.classList.add('fade-in')
      prevQDiv.style.display = 'block'
    }, 500)
  }
  ...
  handleFirstQuestion () {
    ...
    ;[
      ...
      try {
        ...
        console.error('Error showing section:', error.message)
      }
    })
  ...
  handleQuestionCompletion () {
    ...
    const tickLogo = document.getElementById('tickLogo')
    const textMessage = document.getElementById('textMessage')

    tickLogo.style.display = 'block'
...
function hideInitialSections () {
  Object.values(suggestedCourseSection).forEach(id => {
    const element = document.getElementById(id)
    if (element) element.style.display = 'none'
  })
  Object.values(otherSections).forEach(id => {
    const element = document.getElementById(id)
    if (element) element.style.display = 'none'
  })
}
...
const startButton = document.querySelector('input[value="Start assessment"]')
if (startButton) {
  startButton.addEventListener('click', () => this.showQuestions())
}
